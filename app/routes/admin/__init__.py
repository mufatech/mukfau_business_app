from .login import admin_login
from .admin_dashboard import admin_dashboard
from .add_product import add_product, add_supply, stock_balance, profit, supply_cost, all_profits, add_product_supply
from .view_sales_report import sales_report
from .view_supply_report import supply_report, edit_product_supply, delete_product_supply
from .edit_product import edit_product
from .edit_products import edit_products
from .record_sale import record_sale, transaction_details, sale_value
from .edit_supply import edit_supply, delete_supply
from .view_expenses import view_expenses
from .view_product_supplies import product_supplies